  ...
  <header class="flex items-center justify-between px-3 sm:px-5 py-2 sm:py-3 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/50 dark:border-slate-700/50 shrink-0">
    ...
    <div class="flex-1 flex justify-center">
      <div class="flex items-center gap-1.5 sm:gap-2.5">
        <div class="w-7 h-7 rounded-lg bg-gradient-to-br from-rose-400 via-orange-500 to-amber-500 flex items-center justify-center shadow-md shadow-orange-500/25">
      ...
      <div v-if="showShortcuts" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showShortcuts=false">
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-5 max-w-sm w-full shadow-2xl border border-slate-200 dark:border-slate-700">
          <div class="flex items-center justify-between mb-4">
            ...
            <button @click="showShortcuts=false" class="p-1 text-rose-500 hover:text-rose-700 dark:text-orange-400 dark:hover:text-orange-300 rounded"><svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button>
          </div>
...
<style scoped>
.header-btn {
  @apply p-1.5 rounded-md text-rose-500 hover:text-rose-700 dark:text-orange-400 dark:hover:text-orange-300 hover:bg-rose-50/60 dark:hover:bg-rose-950/60 transition-all disabled:opacity-30 disabled:cursor-not-allowed;
}
</style>
