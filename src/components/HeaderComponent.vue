<template>
  <header class="flex items-center justify-between px-5 py-3 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/50 dark:border-slate-700/50 shrink-0">
    <!-- Left: Undo/Redo -->
    <div class="flex items-center gap-1.5 w-[200px]">
    ...
    <div class="flex-1 flex justify-center">
      <div class="flex items-center gap-2.5">
        <div class="w-7 h-7 rounded-lg bg-gradient-to-br from-cyan-400 via-blue-500 to-purple-600 flex items-center justify-center shadow-md shadow-blue-500/25">
          <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
          </svg>
        </div>
        <h1 class="text-base font-semibold text-slate-800 dark:text-slate-100 tracking-tight select-none" style="font-family: Inter, sans-serif">
          Cover-Wave
        </h1>
    ...
    <!-- Right: Controls -->
    <div class="flex items-center gap-1.5 justify-end w-[200px]">
      ...
      <div v-if="showShortcuts" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showShortcuts=false">
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-5 max-w-sm w-full shadow-2xl border border-slate-200 dark:border-slate-700">
          <div class="flex items-center justify-between mb-4">
            ...
            <button @click="showShortcuts=false" class="p-1 text-indigo-500 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 rounded"><svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button>
          </div>
...
<style scoped>
.header-btn {
  @apply p-1.5 rounded-md text-indigo-500 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 hover:bg-indigo-50/60 dark:hover:bg-indigo-950/60 transition-all disabled:opacity-30 disabled:cursor-not-allowed;
}
</style>
