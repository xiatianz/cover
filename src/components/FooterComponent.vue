<template>
  <footer class="py-2 px-5 flex justify-between items-center text-xs text-slate-400 dark:text-slate-500 bg-white/50 dark:bg-slate-900/50 backdrop-blur-md border-t border-slate-200/30 dark:border-slate-700/30 shrink-0">
    <div class="flex items-center gap-2">
      <span>© 2024</span>
      <a href="https://ehon.cn" target="_blank" class="text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-orange-400 transition-colors font-medium">Cover-Wave</a>
    ...
    <div class="flex items-center gap-3">
      <button class="hover:text-rose-600 dark:hover:text-orange-400 transition-colors font-medium" @click="toggleTips">提示</button>
      <button class="hover:text-rose-600 dark:hover:text-orange-400 transition-colors font-medium" @click="showTutorial = true">教程</button>
    </div>
          ...
          <div class="bg-white/95 dark:bg-slate-800/95 backdrop-blur-md rounded-2xl p-6 max-w-lg w-full max-h-[80vh] overflow-y-auto shadow-2xl border border-slate-200/60 dark:border-slate-700/60" @click.stop>
            <div class="flex items-center justify-between mb-5">
              <div class="flex items-center gap-2.5">
                <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-rose-400 via-orange-500 to-amber-500 shadow-md shadow-orange-500/25 flex items-center justify-center shadow-sm">
...
<style scoped>
.btn-ok { @apply px-5 py-2 text-xs font-semibold text-white bg-gradient-to-r from-rose-500 to-orange-500 rounded-lg hover:from-rose-600 hover:to-orange-600 transition-all shadow-md shadow-rose-500/25; }
