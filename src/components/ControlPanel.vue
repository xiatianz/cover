  ...
  <aside class="w-full lg:w-[440px] lg:shrink-0 flex flex-col bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.04),0_4px_16px_rgba(0,0,0,0.04)] dark:shadow-[0_1px_3px_rgba(0,0,0,0.2),0_4px_16px_rgba(0,0,0,0.2)] border border-slate-200/60 dark:border-slate-700/60 overflow-hidden">
    ...
    <div class="px-3 py-1.5 border-b border-slate-200/60 dark:border-slate-700/60 shrink-0">
      <div class="flex items-center gap-1.5 mb-1">
        ...
        <button @click="saveCurrentPreset" class="ml-auto text-[10px] text-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-300 font-medium transition-colors">+ 收藏</button>
      </div>
    ...
    <div class="flex-1 overflow-y-auto min-h-0 divide-y divide-slate-200/60 dark:divide-slate-700/60">
      ...
      <div class="section-block">
        <div class="section-header">
          <svg class="section-icon" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg>
        ...
        <div class="flex gap-1.5 items-center">
          ...
          <a href="https://yesicon.app/" target="_blank" class="btn-accent">图标库</a>
        </div>
        <div class="flex gap-1.5">
          <label for="inputBgImage" class="btn-primary flex-1 text-center cursor-pointer">上传背景</label>
          <input type="file" id="inputBgImage" accept="image/*" @change="onBgUpload" class="hidden" />
          <label for="inputSquareImage" class="btn-primary flex-1 text-center cursor-pointer">上传图标</label>
        ...
        <Transition name="fade">
          <div v-if="state.bgImageUrl || state.squareImageUrl" class="flex gap-1.5">
            <button v-if="state.bgImageUrl" @click="clearBg" class="btn-ghost flex-1">清除背景</button>
            <button v-if="state.squareImageUrl" @click="clearIcon" class="btn-ghost flex-1">清除图标</button>
          </div>
        </Transition>
      </div>
      ...
      <div class="section-block">
        <div class="section-header">
          <svg class="section-icon" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
        ...
        <div class="grid grid-cols-2 gap-1.5">
          ...
          <div class="flex items-center gap-1">
            ...
            <button v-if="state.bgGradient" @click="clearGradient" class="text-[9px] text-slate-400 hover:text-red-500 px-0.5">✕</button>
          </div>
        </div>
        <!-- Row: blur -->
        <div class="flex items-center gap-1">
      ...
      <div class="section-block">
        <div class="grid grid-cols-2 gap-3">
          ...
          <div class="space-y-1.5">
            <div class="section-header">
              <svg class="section-icon" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
              <span class="section-title">文字</span>
            </div>
          ...
          <div class="space-y-1.5">
            <div class="section-header">
              <svg class="section-icon" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
              <span class="section-title">水印</span>
            </div>
      ...
      <!-- 图标样式 -->
      <div v-if="state.squareImageUrl" class="section-block">
        <div class="section-header">
          <svg class="section-icon" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"/></svg>
      ...
      <div class="section-block">
        <div class="section-header">
          <svg class="section-icon" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
          <span class="section-title">导出</span>
        </div>
        <div class="flex gap-1">
          <button v-for="f in formats" :key="f.id" @click="currentFormat = f.id"
            class="format-btn"
            :class="currentFormat === f.id ? 'format-btn-active' : 'format-btn-inactive'">
            {{ f.name }}
          </button>
        </div>
        <div class="flex gap-1.5">
          <button @click="doSave" class="btn-save flex-1">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
            保存图片
          </button>
          <button @click="doCopy" class="btn-secondary">
    ...
    <!-- Bottom bar -->
    <div class="px-3 py-1.5 border-t border-slate-200/60 dark:border-slate-700/60 flex items-center gap-2 shrink-0">
      <button @click="$emit('open-settings')" class="btn-icon" title="平台尺寸">
      ...
      <div class="w-px h-4 bg-slate-200/60 dark:bg-slate-700/60"></div>
...
.btn-primary { @apply px-2.5 py-1.5 rounded-md text-[11px] font-medium text-white bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 transition-all shadow-md shadow-indigo-500/25; }
...
.btn-save { @apply inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium text-white bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 transition-all shadow-md shadow-indigo-500/25; }

.format-btn { @apply flex-1 px-1.5 py-1 rounded-md text-[11px] font-medium transition-all; }
.format-btn-active { @apply bg-gradient-to-r from-indigo-500 to-purple-500 text-white shadow-md shadow-indigo-500/25; }
