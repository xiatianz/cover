    ...
    <Transition appear enter-active-class="transition-opacity duration-300" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition-opacity duration-200" leave-from-class="opacity-100" leave-to-class="opacity-0">
      <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <Transition appear enter-active-class="transition-all duration-300 transform" enter-from-class="opacity-0 scale-95 translate-y-4" enter-to-class="opacity-100 scale-100 translate-y-0" leave-active-class="transition-all duration-200 transform" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95 translate-y-4">
          <div v-if="modelValue" class="relative bg-white/95 dark:bg-stone-800/95 backdrop-blur-sm rounded-2xl w-full max-w-xl shadow-2xl border border-white/60 dark:border-stone-700/60 max-h-[85vh] overflow-hidden flex flex-col" @click.stop>
            <!-- Header -->
            <div class="flex items-center justify-between p-5 border-b border-stone-100 dark:border-stone-700">
              ...
              <button @click="$emit('update:modelValue', false)" class="p-1.5 text-stone-400 hover:text-stone-600 dark:hover:text-stone-300 rounded-lg transition-all">
            ...
            <div class="flex-1 overflow-y-auto p-5">
              <div class="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                <div v-for="p in platforms" :key="p.id" @click="selectPlatform(p)"
                  ...
                  :class="selected?.id === p.id ? 'border-slate-400 bg-slate-50/50 dark:bg-slate-800/50 shadow-[0_0_0_3px_rgba(71,85,105,0.08)]' : 'border-stone-200/80 dark:border-stone-700/80 hover:border-stone-300 dark:hover:border-stone-600 hover:bg-stone-50/50 dark:hover:bg-stone-800/50'">
              ...
              <p v-if="selected" class="mt-4 text-xs text-stone-500 dark:text-stone-400 text-center font-medium">当前：{{ selected.name }} ({{ selected.width }}×{{ selected.height }})</p>
            </div>
            <!-- Footer -->
            <div class="flex justify-end p-4 border-t border-stone-100 dark:border-stone-700">
              <button @click="$emit('update:modelValue', false)" class="px-5 py-2 text-xs font-semibold text-white bg-indigo-400 rounded-lg hover:bg-indigo-500 transition-all shadow-sm">完成</button>
            </div>
          </div>
        </Transition>
      </div>
